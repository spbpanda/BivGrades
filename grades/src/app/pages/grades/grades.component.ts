import { Component, inject } from '@angular/core';
import { GradeComponent } from '../../components/grade/grade.component';
import { MatDialog } from '@angular/material/dialog';
import { GradeInfoComponent } from '../../components/grade/grade-info/grade-info.component';

export interface Grade {
  name: string;
  description: string;
  characteristics: string[];
  hardSkills: Map<string, string[]>;
  softSkills: Map<string, string[]>;
  projectSkills: Map<string, string[]>;
  howToRate: string[];
  imageUrl: string;
}

@Component({
  selector: 'app-grades',
  standalone: true,
  imports: [GradeComponent],
  templateUrl: './grades.component.html',
  styleUrl: './grades.component.scss',
})
export class GradesComponent {
  dialog = inject(MatDialog);
  grades: Grade[] = [
    {
      name: 'Стажер',
      description:
        'Начальная ступень в карьере frontend-разработчика. На этом уровне сотрудник только знакомится с основами веб-разработки, учится работать в команде и выполняет простые задачи под руководством ментора. Зачастую, после 2-х месяцев работы получают позицию Junior -.',
      characteristics: [
        'Минимальный коммерческий опыт (либо его отсутствие).',
        'Требуется постоянная поддержка и код-ревью.',
        'Понимание основных принципов веб-разработки (либо желание их изучить).',
        'Акцент на обучение, а не на самостоятельную работу.',
      ],
      hardSkills: new Map([
        [
          'HTML/CSS',
          [
            'понимание структуры HTML-документа',
            'базовые стили (селекторы, box model, flexbox/grid)',
          ],
        ],
        ['JavaScript', ['базовые типы данных', 'функции']],
        [
          'Git',
          ['базовые команды (clone, commit, push, pull)', 'работа в ветках'],
        ],
      ]),
      softSkills: new Map([
        [
          'Обучаемость',
          [
            'способность быстро усваивать новую информацию',
            'способность воспринимать обратную связь и исправлять ошибки',
          ],
        ],
        [
          'Коммуникабельность',
          ['способность работать в команде', 'следовать инструкциям ментора'],
        ],
        ['Тайм-менеджмент', ['оценка своих сил и сроков выполнения задач']],
      ]),
      projectSkills: new Map([
        [
          'Работа на проекте',
          [
            'выполнение мелких задач под руководством',
            'понимание комментариев к своему коду',
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – проверка через тестовое задание (верстка макета + простой JS), задачи по проекту.',
        'Soft Skills – наблюдение за скоростью усвоения информации, реакцией на правки в код-ревью.',
        'Project Skills – анализ качества выполнения первых задач, умения работать с Git (не ломает ветки)',
      ],
      imageUrl: '/assets/intern.png',
    },
    {
      name: 'Junior -',
      description:
        'Первый полноценный уровень в карьере фронтенд-разработчика. Специалист уже может самостоятельно выполнять простые задачи, но ещё требует регулярного контроля и менторской поддержки. Основная цель — закрепление базовых навыков и постепенный переход к самостоятельной работе.',
      characteristics: [
        'Может работать без постоянного надзора, но нуждается в код-ревью.',
        'Выполняет задачи по готовым ТЗ.',
        'Начинает понимать процессы разработки в команде.',
        'Допускает ошибки, но способен их исправлять с подсказками.',
      ],
      hardSkills: new Map([
        [
          'HTML/CSS',
          [
            'адаптивная и кросс-браузерная верстка',
            'препроцессоры (SASS/SCSS) на базовом уровне',
          ],
        ],
        [
          'JavaScript',
          [
            'ES6+ (стрелочные функции, деструктуризация, шаблонные строки)',
            'Работа с DOM (создание/изменение элементов)',
            'Обработка событий',
            'Базовое понимание TypeScript',
            'Знакомство с REST API',
          ],
        ],
        [
          'Фреймворки',
          [
            'Знакомство с любым фреймворком (React/Vue/Angular) на уровне создания компонентов',
          ],
        ],
        [
          'Инструменты',
          [
            'Основы отладки в DevTools',
            'npm/yarn (установка зависимостей, запуск скриптов)',
          ],
        ],
      ]),
      softSkills: new Map([
        [
          'Коммуникация',
          [
            'Четкое формулирование вопросов',
            'Умение описывать проблему перед обращением за помощью',
            'Следование стандартам команды',
          ],
        ],
        [
          'Работа в команде',
          ['Участие в ежедневных стендапах', 'Понимание agile-процессов', 'Восприятие обратной связи'],
        ],
        ['Самоорганизация', ['Приоритизация работы', 'Отслеживание своего прогресса']],
      ]),
      projectSkills: new Map([
        [
          'Самостоятельное выполнение задач',
          [
            'Исправление багов',
            'Реализация простой бизнес-логики',
          ],
        ],
        [
          'Понимание процессов',
          [
            'Работа по готовым ТЗ',
            'Следование стандартам кода проекта',
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – проверка через тестовое задание (верстка макета), задачи по проекту, техническое собеседование (вопросы по JS, фреймворкам)',
        'Soft Skills – наблюдение за качеством коммуникации в команде, реакцией на правки в код-ревью, способностью самостоятельно решать проблемы',
        'Project Skills – анализ качества выполнения задач, участия в командных процессах.',
      ],
      imageUrl: '/assets/junior1.png',
    },
    {
      name: 'Junior',
      description:
        'Уверенный начинающий разработчик, способный самостоятельно решать типовые задачи. Может работать с минимальным контролем, понимает процессы разработки и начинает вносить вклад в архитектурные решения. Основная цель — переход к решению более сложных задач и подготовка к уровню Junior +.',
      characteristics: [
        'Самостоятельно выполняет большинство типовых задач.',
        'Понимает бизнес-логику своих задач.',
        'Может помогать стажёрам и Junior-.',
        'Допускает ошибки, но способен их исправлять с подсказками.',
      ],
      hardSkills: new Map([
        [
          'Продвинутый JavaScript/TypeScript',
          [
            'Основы TypeScript (интерфейсы, типы)',
            'Работа с замыканиями, контекстом (this)',
            'Понимание работы Event Loop',
          ],
        ],
        [
          'Фреймворки',
          [
            'Уверенная работа с Angular/React',
            'Понимание жизненных циклов компонентов/хуки',
            'Основы работы с стейтменеджерами NgXs/NgRx',
          ],
        ],
        [
          'Работа с API',
          [
            'REST (axios/fetch/httpclient)',
            'Обработка ошибок',
            'Аутентификация (JWT)',
          ],
        ],
        [
          'Инструменты',
          [
            'Начальные знания в тестировании (unit/integration)',
          ],
        ],
      ]),
      softSkills: new Map([
        [
          'Коммуникация',
          [
            'Чёткое формулирование технических решений',
            'Умение аргументировать свой выбор',
            'Следование стандартам команды',
          ],
        ],
        [
          'Работа в команде',
          ['Участие в ежедневных стендапах', 'Участие в код-ревью', 'Участие в технических обсуждениях'],
        ],
        [
          'Профессиональное развитие', 
          ['Самостоятельное изучение новых технологий', 'Анализ своих ошибок', 'Умение оценивать сложность задач']
        ],
      ]),
      projectSkills: new Map([
        [
          'Самостоятельное выполнение задач',
          [
            'Сложных компонентов',
            'Маршрутизации',
            'Интеграции с API',
            'Ведение технических долгов проекта',
          ],
        ],
        [
          'Оптимизация',
          [
            'Рефакторинг legacy-кода',
            'Улучшение производительности компонентов',
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – проверка через реализацию SPA с API-интеграцией, техническое собеседование с вопросами по (фреймворкам, инструментам), разбор чужого кода.',
        'Soft Skills – наблюдение за участием в планировании, качеством коммуникации, скоростью решения проблем.',
        'Project Skills – анализ качества сложности выполненных задач, качества кода, вклада в развитие проекта.',
      ],
      imageUrl: '/assets/junior2.png',
    },
    {
      name: 'Junior +',
      description:
        'Продвинутый junior-разработчик, демонстрирующий признаки готовности к переходу на middle-уровень. Способен решать сложные задачи, вносить архитектурные предложения и самостоятельно находить оптимальные решения. На этом уровне разработчик начинает мыслить не только в рамках конкретной задачи, но и в контексте всего проекта.',
      characteristics: [
        'Самостоятельно решает нетривиальные задачи',
        'Активно участвует в проектировании решений',
        'Понимает и учитывает бизнес-контекст задач',
        'Начинает работать над качеством кода всей команды',
      ],
      hardSkills: new Map([
        [
          'Продвинутый JavaScript/TypeScript',
          [
            'Оптимизация производительности',
            'Продвинутые типы TypeScript (Generics, Utility Types)',
          ],
        ],
        [
          'Фреймворки',
          [
            'Кастомные хуки/pipe/directives',
            'State-менеджмент',
            'Уверенное знания одного из фреймворков (React/Angular)',
          ],
        ],
        [
          'Работа с API',
          [
            'Кеширование запросов',
            'Уверенное знание POST/GET/PUT/DELETE запросов',
          ],
        ],
        [
          'Инструменты',
          [
            'Знание микросервисной/монолит архитектуры',
            'ООП/ФП',
          ],
        ],
      ]),
      softSkills: new Map([
        [
          'Техническое общение',
          [
            'Умение сказать "нет" плохим решениям',
            'Проведение технических обсуждений',
          ],
        ],
        [
          'Работа в команде',
          ['Проведение код-ревью для junior-разработчиков', 'Участие в код-ревью', 'Активное участие в планировании'],
        ],
        [
          'Профессиональное развитие', 
          ['Написание технической документации', 'Написаное самодокументированного кода']
        ],
      ]),
      projectSkills: new Map([
        [
          'Разработка сложных фич',
          [
            'Система прав доступа',
            'Сложные анимации',
          ],
        ],
        [
          'Архитектурные решения',
          [
            'Проектирование новых модулей',
            'Оптимизация сборки',
          ],
        ],
        [
          'Качество кода',
          [
            'Контроль технического долга'
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – проверка через реализацию SPA с API-интеграцией, оптимизация производительности, техническое собеседование с вопросами по (фреймворкам, инструментам), разбор чужого кода.',
        'Soft Skills – наблюдение за участием в принятии решений, качеством менторства, проактивностью.',
        'Project Skills – анализ вклада в архитектуру, качества проведённых код-ревью, участия в улучшении процессов.',
      ],
      imageUrl: '/assets/junior3.png',
    },
    {
      name: 'Middle -',
      description:
        'Переходная ступень от Junior+ к полноценному Middle. Разработчик на этом уровне уже способен самостоятельно решать сложные задачи, но ещё нуждается в консультациях по архитектурным решениям. Основная характеристика — баланс между технической экспертизой и растущей ответственностью за проект.',
      characteristics: [
        'Полная автономность в решении типовых задач',
        'Глубже понимание бизнес-контекста',
        'Ответственность за качество кода в своём модуле',
        'Способность оценивать риски технических решений',
      ],
      hardSkills: new Map([
        [
          'Продвинутый TypeScript',
          [
            'Conditional Types',
            'Type Inference',
            'Type Guards',
          ],
        ],
        [
          'Архитектура приложений',
          [
            'Паттерны проектирования',
            'Чистая архитектура'
          ],
        ],
        [
          'Производительность',
          [
            'Оптимизация рендеринга',
            'Lazy loading модулей',
          ],
        ],
        [
          'Тестирование',
          [
            'Интеграционные тесты',
            'Mocking API',
          ],
        ],
      ]),
      softSkills: new Map([
        [
          'Техническое общение',
          [
            'Проведение технических интервью под примсотром тех. специалиста',
            'Формализация best practices',
          ],
        ],
        [
          'Коммуникация',
          ['Общение с backend-разработчиками на равных', 'Участие в оценке фич'],
        ],
        [
          'Менторство', 
          ['Планирование развития junior-разработчиков']
        ],
      ]),
      projectSkills: new Map([
        [
          'Роль в разработке',
          [
            'Интеграции со сторонними сервисами',
            'Сложные анимации',
          ],
        ],
        [
          'Оптимизация',
          [
            'Миграция на новые версии фреймворков',
            'Устранение узких мест производительности',
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – проверка глубины понимания выбранного стека, качество архитектурных предложений.',
        'Soft Skills – наблюдение за участием в развитии команды, вклад в снижение технического долга, количество и значимость внедрённых улучшений.',
        'Project Skills – анализ коммуникации с продукт-менеджерами, качества проведённых код-ревью, участия в улучшении процессов.',
      ],
      imageUrl: '/assets/middle1.jpg',
    },
    {
      name: 'Middle',
      description:
        'Самостоятельный и зрелый специалист, способный полностью владеть своим направлением разработки. Основная характеристика — способность не только решать сложные технические задачи, но и осознанно выбирать оптимальные решения с учетом долгосрочной перспективы проекта.',
      characteristics: [
        'Полная ответственность за технические решения',
        'Способность предвидеть последствия архитектурных решений',
        'Активное влияние на техническую стратегию проекта',
        'Умение балансировать между перфекционизмом и бизнес-требованиями',
      ],
      hardSkills: new Map([
        [
          'Экспертный уровень TypeScript',
          [
            'Сложные generic-типы',
            'Type inference системы',
            'Кастомные type guards и утилиты',
          ],
        ],
        [
          'Архитектурные паттерны',
          [
            'Микросервисная архитектура фронтенда',
            'Продвинутые знания в State-менеджере',
            'Event-driven архитектура',
          ],
        ],
        [
          'Тестирование',
          [
            'Нагрузочное тестирование UI',
            'Unit tests',
          ],
        ],
      ]),
      softSkills: new Map([
        [
          'Техническое общение',
          [
            'Формирование технического видения',
            'Управление техническим долгом',
          ],
        ],
        [
          'Коммуникация',
          ['Эффективное взаимодействие с CTO/архитекторами', 'Проведение технических презентаций'],
        ],
        [
          'Менторство', 
          ['Разработка программ обучения', 'Техническое сопровождение нескольких разработчиков']
        ],
      ]),
      projectSkills: new Map([
        [
          'Лидерство в реализации',
          [
            'Сложных кросс-функциональных фич',
            'Миграций на новые технологии',
          ],
        ],
        [
          'Качество процессов',
          [
            'Управление техническими рисками',
            'Патенты на технические решения',
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – проверка экспертного владения стеком технологий, качества архитектурных предложений.',
        'Soft Skills – наблюдение за участием в развитии команды, вклад в снижение технического долга, количество и значимость внедрённых улучшений.',
        'Project Skills – анализ снижения времени разработки, количество внедрённых улучшений, повышение стабильности продукта.',
      ],
      imageUrl: '/assets/middle2.jpg',
    },
    {
      name: 'Middle +',
      description:
        'Эксперт уровня Middle, демонстрирующий готовность к переходу на Senior позицию. Ключевое отличие — способность не только решать сложные технические задачи, но и формировать техническую стратегию для всего фронтенд-направления.',
      characteristics: [
        'Техническое лидерство в масштабах компании',
        'Ответственность за кросс-командные инициативы',
        'Умение проектировать системы с 3-5 летней перспективой',
        'Баланс между инновациями и техническим консерватизмом',
      ],
      hardSkills: new Map([
        [
          'Архитектурные решения',
          [
            'Микрофронтенды (Module Federation, Web Components)',
            'Websockets',
            'GraphQL',
          ],
        ],
        [
          'Инструменты экспертного уровня',
          [
            'Performance tracing (WebPageTest, Lighthouse CI)',
            'Advanced debugging (Memory leaks, CPU profiling)',
          ],
        ],
        [
          'Безопасность',
          [
            'CSP политики',
            'Защита от XSS/CSRF',
            'Аутентификация/авторизация',
          ],
        ],
      ]),
      softSkills: new Map([
        [
          'Стратегическое мышление',
          [
            'Техническое прогнозирование',
            'Roadmap планирование',
          ],
        ],
        [
          'Лидерство',
          ['Формирование технической культуры', 'Построение инженерных практик', 'Управление техническими конфликтами'],
        ],
        [
          'Коммуникация', 
          ['Презентации для нетехнических стейкхолдеров']
        ],
        [
          'Менторство', 
          ['Разработка программ обучения', 'Техническое сопровождение нескольких разработчиков']
        ],
      ]),
      projectSkills: new Map([
        [
          'Лидерство в реализации',
          [
            'Разработке платформенных решений',
          ],
        ],
        [
          'Инновации/Качество',
          [
            'Система метрик качества кода',
            'Proof-of-Concept новых технологий',
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – проверка вклада в технологический стек компании.',
        'Soft Skills – наблюдение за участием в развитии продукта и общение с клиентом, влияние на компанию.',
        'Project Skills – анализ продуктивности, построение процессов, технического лидерства.',
      ],
      imageUrl: '/assets/middle3.jpg',
    },
    {
      name: 'Senior -',
      description:
        'Переходная ступень от Middle+ к полноценному Senior. Разработчик на этом уровне уже обладает экспертизой Senior, но ещё нарабатывает опыт стратегического влияния на бизнес. Основная характеристика — способность соединять глубокую техническую экспертизу с бизнес-ориентированностью.',
      characteristics: [
        'Переход от решения технических проблем к их предупреждению',
        'Ответственность за технологические решения на уровне продукта',
        'Формирование технического видения направления',
        'Ведение нескольких проектов одновременно',
      ],
      hardSkills: new Map([
        [
          'Архитектурные решения',
          [
            'Проектирование fault-tolerant систем (отказоустойчивость)',
            'Реализация graceful degradation (когда элемент не ломают работу всей системы)',
            'Оптимизация для экстремальных условий (медленные сети, слабые устройства)',
          ],
        ],
        [
          'Фреймворки',
          [
            'Эксперное знание фреймворков (Angular/React/Vue)',
          ],
        ],
        [
          'Безопасность',
          [
            'Аудит безопасности существующих решений',
          ],
        ],
      ]),
      softSkills: new Map([
        [
          'Стратегическое мышление',
          [
            'Техническое прогнозирование',
            'Roadmap планирование',
            'Оценка технических инвестиций'
          ],
        ],
        [
          'Лидерство',
          ['Разрешение кросс-командных конфликтов', 'Построение процессов knowledge sharing', 'Поддержка инженерной культуры'],
        ],
        [
          'Менторство', 
          ['Собеседования и разработка тестовых задач']
        ],
      ]),
      projectSkills: new Map([
        [
          'Лидерство в реализации',
          [
            'Разработке платформенных решений',
          ],
        ],
        [
          'Инновации/Качество',
          [
            'Система метрик качества кода',
            'Proof-of-Concept новых технологий',
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – проверка вклада в технологический стек компании, глубина понимания системных компромиссов.',
        'Soft Skills – наблюдение за участием в развитии продукта и общение с клиентом, влияние на компанию.',
        'Project Skills – анализ продуктивности, построение процессов, технического лидерства.',
      ],
      imageUrl: '/assets/senior1.jpg',
    },
    {
      name: 'Senior',
      description:
        'Зрелый технический лидер, способный влиять не только на техническую, но и на бизнес-стратегию компании. Основная характеристика — способность соединять глубокую техническую экспертизу с пониманием бизнес-процессов и формировать технологическое видение компании.',
      characteristics: [
        'Полная ответственность за технологическое направление',
        'Способность предвидеть технологические тренды',
        'Формирование инженерной культуры в компании',
        'Участие в стратегических бизнес-решениях',
      ],
      hardSkills: new Map([
        [
          'Системная архитектура',
          [
            'Проектирование кросс-платформенных решений',
            'Решения для глобального масштабирования',
          ],
        ],
        [
          'Экстремальная оптимизация',
          [
            'Adaptive resource delivery - это оптимизация загрузки веб-страниц за счёт динамического выбора и доставки ресурсов (изображений, видео, скриптов, стилей) в зависимости от условий пользователя.',
            'Offline-first архитектуры - Offline-first архитектура — это подход к разработке веб- и мобильных приложений, при котором приложение работает без интернета или при плохом соединении, а синхронизация с сервером происходит, когда подключение восстанавливается.',
          ],
        ],
      ]),
      softSkills: new Map([
        [
          'Стратегическое лидерство',
          [
            'Формирование технического roadmap компании',
          ],
        ],
        [
          'Коммуникация и влияние',
          ['Выпуск статей', 'Выступления в компании или за пределами'],
        ],
      ]),
      projectSkills: new Map([
        [
          'Лидерство в реализации',
          [
            'Разработке платформенных решений',
          ],
        ],
        [
          'Инновации/Качество',
          [
            'Система метрик качества кода',
            'Proof-of-Concept новых технологий - новые технологии реализуются на практике',
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – проверка вклада в технологический стек компании, глубина понимания системных компромиссов.',
        'Soft Skills – наблюдение за участием в развитии продукта и общение с клиентом, влияние на компанию.',
        'Project Skills – анализ продуктивности, построение процессов, технического лидерства.',
      ],
      imageUrl: '/assets/senior2.jpg',
    },
    {
      name: 'Senior +',
      description:
        'Вершина технической карьеры разработчика, где техническая экспертиза трансформируется в стратегическое влияние на бизнес и индустрию. Это уровень архитектурных решений, формирующих будущее не только компании, но и всей отрасли.',
      characteristics: [
        'Формирование технологического видения компании на 5-10 лет',
        'Создание новых технологических продуктов',
        'Признанная экспертиза',
        'Ответственность за технологическую стратегию как конкурентное преимущество',
      ],
      hardSkills: new Map([
        [
          'Архитектура нового поколения',
          [
            'Проектирование self-adaptive систем',
            'Disaster recovery стратегии'
          ],
        ],
        [
          'Инновации',
          [
            'Создание новых парадигм разработки',
            'Разработка стандартов и протоколов',
          ],
        ],
      ]),
      softSkills: new Map([
        [
          'Стратегическое лидерство',
          [
            'Формирование технологического ландшафта компании',
          ],
        ],
        [
          'Развитие экосистемы',
          [
            'Создание технических сообществ',
            'Технический консалтинг',
          ],
        ],
        [
          'Влияние',
          [
            'Формирование технологической политики',
            'Взаимодействие с регуляторами',
          ],
        ],
      ]),
      projectSkills: new Map([
        [
          'Лидерство в',
          [
            'Трансформационных инициативах',
            'Создании новых технологических направлений',
            'Кризисных ситуациях',
          ],
        ],
        [
          'Инновации/Качество',
          [
            'Управление технологическими рисками',
            'Обеспечение технологического суверенитета (без зависимости от внешних поставщиков)',
          ],
        ],
      ]),
      howToRate: [
        'Hard Skills – ???.',
        'Soft Skills – ???.',
        'Project Skills – ???.',
      ],
      imageUrl: '/assets/senior3.jpg',
    },
  ];

  openGradeInfo(grade: Grade) {
    this.dialog.open(GradeInfoComponent, {
      data: grade,
    });
  }
}
